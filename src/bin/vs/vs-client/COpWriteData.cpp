...
bool CVSClient::WriteResults(CXMLElement* p_root)
{
    CSmallString results_file_name;

    // following argument is optional
    if(ActionRequest.GetParameterKeyValue("results",results_file_name) == false) return(true);

    CXMLElement* p_res = p_root->CreateChildElement("RESULTS");
    if(p_res == NULL) {
        ES_ERROR("unable to create results element");
        return(false);
    }

    CPrmFile    prmfile;

    if(prmfile.Read(results_file_name) == false) {
        ES_ERROR("unable to read results file");
        return(false);
    }

    if(prmfile.OpenSection("results") == false) {
        ES_ERROR("unable to open [results] section in results file");
        return(false);
    }
    ...
    while(result) {
        ...
        if(prmfile.GetLineKey(key) == false) {
            ES_ERROR("unable to get line key");
            return(false);
        }
        if(prmfile.GetLineValue(value) == false) {
            ES_ERROR("unable to get line value");
            return(false);
        }
        p_res->SetAttribute(key,value);
