...
bool CVSClient::GetData(CSmallString& molid)
{
    ...
    // create command
    CClientCommand* p_command = CreateCommand(Operation_GetData);
...
bool CVSClient::WriteTemplate(CXMLElement* p_root)
{
    CSmallString template_file_name;

    // following argument is optional
    if(ActionRequest.GetParameterKeyValue("template",template_file_name) == false) return(true);

    CSmallString data_file_name;

    // following argument is optional
    if(ActionRequest.GetParameterKeyValue("data",data_file_name) == false) return(true);

    CXMLElement* p_res = p_root->CreateChildElement("TEMPLATE");
    if(p_res == NULL) {
    ...
    if(prmfile.Read(template_file_name) == false) {
        ES_ERROR("unable to read template file");
        return(false);
    }

    if(prmfile.OpenSection("template") == false) {
        ES_ERROR("unable to open [template] section in template file");
        return(false);
    }

    bool result = prmfile.FirstLine();

    while(result) {
        CSmallString key;
        if(prmfile.GetLine(key) == false) {
            ES_ERROR("unable to get line key");
            return(false);
        }
        p_res->SetAttribute(key,"");
        result = prmfile.NextLine();
    }
    return(true);
}
...
bool CVSClient::ReadData(CXMLElement* p_root)
{
    CSmallString template_file_name;
    ...
    CXMLElement* p_res = p_root->GetFirstChildElement("DATA");
    if(p_res == NULL) {
        ES_ERROR("unable to open DATA element");
        return(false);
    }

    FILE* p_fout = fopen(data_file_name,"wb");
    if(p_fout == NULL) {
        CSmallString error;
        error << "unable to open data file (" << data_file_name << ")";
        ES_ERROR(error);
        return(false);
    }
