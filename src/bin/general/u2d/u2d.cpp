...
bool ProcessFile(const char* p_name)
{
    ...
    for(unsigned int i=0; i<size; i++) {
        bool result = true;
        if( buffer[i] == 0x0a ) {
            result &= fputc(0x0d,file) != EOF;
            result &= fputc(0x0a,file) != EOF;
        } else {
            result &= fputc(buffer[i],file) != EOF;
        }
        if( result == false ) {
            if( Error == false) fprintf(stderr,"\n");
...
bool ProcessStream(void)
{
    ...
    while( (buffer = fgetc(stdin)) != EOF ) {
        bool result = true;
        if( buffer == 0x0a ) {
            result &= fputc(0x0d,stdout) != EOF;
            result &= fputc(0x0a,stdout) != EOF;
        } else {
            result &= fputc(buffer,stdout) != EOF;
        }
        if( result == false ) {
            if( Error == false) fprintf(stderr,"\n");
            fprintf(stderr," ERROR: Can't write into the output stream\n");
