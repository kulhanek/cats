...
#include "TopTraj2ARdfB.hpp"

//==============================================================================
//------------------------------------------------------------------------------
//==============================================================================

CTopTraj2ARdfB::CTopTraj2ARdfB(void)
{
...
int CTopTraj2ARdfB::Init(int argc,char* argv[])
{
...
bool CTopTraj2ARdfB::Finalize(void)
{
    if( Options.GetOptVerbose() ) {
...
bool CTopTraj2ARdfB::Calculate(int residA,int residB)
{
    ...
    if( p_res1->GetNumberOfAtoms() <= 3 ){
        CSmallString error;
        error << "solute residue " << residA+1 << " does not contain three or more atoms";
        ES_ERROR(error);
        return(false);
    }
    ...
    for(int i=0; i < p_res1->GetNumberOfAtoms(); i++) {
        ...
        PLN[0][0] += mass*(pos.x - com1.x)*(pos.x - com1.x);
        PLN[1][1] += mass*(pos.y - com1.y)*(pos.y - com1.y);
        PLN[2][2] += mass*(pos.z - com1.z)*(pos.z - com1.z);

        PLN[0][1] += mass*(pos.x - com1.x)*(pos.y - com1.y);
        PLN[0][2] += mass*(pos.x - com1.x)*(pos.z - com1.z);
        PLN[1][2] += mass*(pos.y - com1.y)*(pos.z - com1.z);
    }
    ...
    //! now solve eigenproblem
    info = CLapack::syev('V','L',PLN,EigV);
    ...
    // calculate angle ---------------------------
    double angle = Angle(ivec1,ivec2);
    if( angle > M_PI/2.0 ){
...
void CTopTraj2ARdfB::PrintHeader(FILE* p_fout)
{
    CSmallTimeAndDate dt;
    dt.GetActualTimeAndDate();

    fprintf(p_fout,"# ==============================================================================\n");
    fprintf(p_fout,"# toptraj2ardfB (CATs utility) started at %s\n",(const char*)dt.GetSDateAndTime());
    fprintf(p_fout,"# ==============================================================================\n");
    fprintf(p_fout,"#\n");
